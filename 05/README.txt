...
# 4**: Смысл тот же, что и в задании 3, только программа будет принимать последовательность аргументов которые помимо чисел принимают операторы, кроме того, числовой ряд будет от 0 до 100 включительно. Пример:
"twenty one", "plus", "forty two", "minus", "four", "multiply", "five", результат: 21 + 42 - 4 * 5 = 43.
...
# 11: Написать рекурсивную функцию, которая будет считать произведение переданного ей в качестве аргумента список целых чисел. Пример: ввод: [42, 78, 113], результат: 42 * 72 * 113 = 370188.